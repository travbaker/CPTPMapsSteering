...
#compare with result derived by Bowles et. al., Eq. (7):
bow_1=[]
bow_3=[]
start=time.time()
nbow=100
...
def least_eig(r,theta,alpha,a_z):
    rho=(1/4)*(qt.tensor(qt.qeye(2), qt.qeye(2)) + qt.tensor(a_z*qt.sigmaz(), qt.qeye(2))\
               - r*np.sin(theta)*np.cos(alpha)*qt.tensor(qt.sigmax(),qt.sigmax()) - \
                   r*np.sin(theta)*np.sin(alpha)*qt.tensor(qt.sigmay(),qt.sigmay()) - \
